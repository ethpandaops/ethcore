...
// BlsToExecutionProcessor defines the interface for processing BLS to execution change messages.
type BlsToExecutionProcessor interface {
	pubsub.Processor[*pb.SignedBLSToExecutionChange]
}

// DefaultBlsToExecutionProcessor handles BLS to execution change messages.
type DefaultBlsToExecutionProcessor struct {
	ForkDigest [4]byte
...
func (p *DefaultBlsToExecutionProcessor) Topic() string {
	return BlsToExecutionChangeTopic(p.ForkDigest)
}

func (p *DefaultBlsToExecutionProcessor) AllPossibleTopics() []string {
	return []string{p.Topic()}
}

func (p *DefaultBlsToExecutionProcessor) Decode(ctx context.Context, data []byte) (*pb.SignedBLSToExecutionChange, error) {
...
func (p *DefaultBlsToExecutionProcessor) Validate(ctx context.Context, change *pb.SignedBLSToExecutionChange, from string) (pubsub.ValidationResult, error) {
...
func (p *DefaultBlsToExecutionProcessor) Process(ctx context.Context, change *pb.SignedBLSToExecutionChange, from string) error {
	if p.Handler == nil {
...
func (p *DefaultBlsToExecutionProcessor) GetTopicScoreParams() *pubsub.TopicScoreParams {
	// Return nil to use default/no scoring for now
	// Users can override this by providing their own processor implementation
	return nil
}

// Compile-time check that DefaultBlsToExecutionProcessor implements BlsToExecutionProcessor.
var _ BlsToExecutionProcessor = (*DefaultBlsToExecutionProcessor)(nil)
