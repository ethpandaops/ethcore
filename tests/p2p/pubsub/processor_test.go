...
type mockProcessor struct {
	...
	processError    error
	decodeResult    string
	validateResult  ethpubsub.ValidationResult
	processingDelay time.Duration
	scoreParams     *ethpubsub.TopicScoreParams
...
func newMockProcessor(topic string) *mockProcessor {
	return &mockProcessor{
		topic:          topic,
		validateResult: ethpubsub.ValidationAccept,
		decodeResult:   "",
	}
}
...
func (m *mockProcessor) Validate(ctx context.Context, msg string, from string) (ethpubsub.ValidationResult, error) {
	atomic.AddInt32(&m.validateCalls, 1)

	if m.validateError != nil {
		return ethpubsub.ValidationReject, m.validateError
	}

	// Default behavior based on message content
	if msg == "invalid" {
		return ethpubsub.ValidationReject, nil
	}
	if msg == "ignore" {
		return ethpubsub.ValidationIgnore, nil
	}
...
func (m *mockProcessor) setValidateResult(result ethpubsub.ValidationResult) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
type mockMultiProcessor struct {
	...
	validateResult  ethpubsub.ValidationResult
	processingDelay time.Duration
	scoreParams     map[string]*ethpubsub.TopicScoreParams
...
func newMockMultiProcessor(topics []string) *mockMultiProcessor {
	return &mockMultiProcessor{
		topics:         topics,
		scoreParams:    make(map[string]*ethpubsub.TopicScoreParams),
...
func (m *mockMultiProcessor) Validate(ctx context.Context, topic string, msg string, from string) (ethpubsub.ValidationResult, error) {
	m.mu.Lock()
	...
	if validateError != nil {
		return ethpubsub.ValidationReject, validateError
	}
...
func TestProcessorInterface(t *testing.T) {
	...
	var _ ethpubsub.Processor[string] = processor
...
func TestProcessorMetrics(t *testing.T) {
	...
	metrics := ethpubsub.NewProcessorMetrics(log)
	...
	metrics.RecordValidationResult(ethpubsub.ValidationAccept)
	metrics.RecordValidationResult(ethpubsub.ValidationReject)
	metrics.RecordValidationResult(ethpubsub.ValidationIgnore)
	stats = metrics.GetStats()
...
func TestProcessorMetricsTopicMetrics(t *testing.T) {
	...
	topicMetrics1.RecordValidationResult(ethpubsub.ValidationAccept)
...
func TestValidationResultString(t *testing.T) {
	tests := []struct {
		result   ethpubsub.ValidationResult
		expected string
	}{
		{ethpubsub.ValidationAccept, "accept"},
		{ethpubsub.ValidationReject, "reject"},
		{ethpubsub.ValidationIgnore, "ignore"},
		{ethpubsub.ValidationResult(999), "unknown"},
	}
	...
	t.Run("error_handling", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if result == ethpubsub.ValidationAccept {
					err = processor.Process(ctx, decoded, testPeer)
...
func TestProcessorValidation(t *testing.T) {
	...
	tests := []struct {
		name           string
		message        string
		expectedResult ethpubsub.ValidationResult
		expectError    bool
	}{
		{
			name:           "accept_valid_message",
			message:        "valid message",
			expectedResult: ethpubsub.ValidationAccept,
			expectError:    false,
		},
		{
			name:           "reject_invalid_message",
			message:        "invalid",
			expectedResult: ethpubsub.ValidationReject,
			expectError:    false,
		},
		{
			name:           "ignore_ignorable_message",
			message:        "ignore",
			expectedResult: ethpubsub.ValidationIgnore,
			expectError:    false,
		},
	}
	...
	t.Run("validation_result_consistency", func(t *testing.T) {
		processor := newMockProcessor("test-topic")
		processor.setValidateResult(ethpubsub.ValidationIgnore)
...
func TestProcessorMetricsRaceConditions(t *testing.T) {
	...
	t.Run("separate_metrics_instances", func(t *testing.T) {
		...
		results := make([]ethpubsub.ProcessorStats, numGoroutines)
		...
		for i := 0; i < numGoroutines; i++ {
			go func(id int) {
				...
				for j := 0; j < numOperations; j++ {
					metrics.RecordMessage()
					metrics.RecordProcessed()
					metrics.RecordValidationResult(ethpubsub.ValidationResult(j % 3))
				}
...
func TestProcessorPerformanceMetrics(t *testing.T) {
	...
	freshMetrics := ethpubsub.NewProcessorMetrics(log)
